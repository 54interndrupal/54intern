...
Users with the "Clone any node" or "Clone own nodes" permission can utilize
this functionality. An action link will appear on node pages with the text
"Clone content". Depending on the setting you choose there are two possible 
actions that will occur when you click that link. Each of these is a different
cloning "method".

The default method works by pre-populating the node form, rather than immediately
saving a copy of the original node to the database.  Thus, your node will not be
...
a node unless they have permission to create new nodes of that type based on a
call to node_access().
...
especially if you are using a complex or custom node type, you should
...
This module makes no attempt to respect field-level permissions.
You should test your configuration carefully or not allow access
to the clone functionality for non-admins if you rely on field-level
permissions for important site features.
