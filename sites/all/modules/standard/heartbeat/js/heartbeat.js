(function($) {
	...
	Drupal.heartbeat.pollMessages = function(stream) {
	  ...
	  // Lookup the streams and poll them.
	  if (stream == undefined) {
	    $('.heartbeat-stream').each(function(i) {
	      var streamId = $(this).attr('id');
	      Drupal.heartbeat._pollMessages("#" + streamId);
	    });
	  }
	  else {
	    return Drupal.heartbeat._pollMessages('#heartbeat-stream-' + stream);
	  }
 ...
 * current stream.
 */
Drupal.heartbeat._pollMessages = function(stream_selector) {

  // Only trigger if a selector is met.
	  if ($(stream_selector).length > 0) {

	    var pollType = parseInt(Drupal.settings.heartbeatPollTypes[n]);
...
Drupal.heartbeat.prependMessagesActions = function (data) {
  ...
  Drupal.heartbeat.wait(element, '.heartbeat-more-messages-wrapper');
	 ...
	 * Remove element.
	 */
	$.fn.heartbeatNotifyStreams = function(data) {
