...
class usermodel {
	...
	function add_user($username, $password, $email, $uid = 0, $questionid = '', $answer = '', $regip = '') {
		...
		//$this->db->query("INSERT INTO test fields SET content='$uid'");
		
	

		//注册后同步登录
		//mysql_select_db("shixiquan");
		//$data = $this->db->fetch_first("SELECT max(uid) as muid FROM `shixiquan`.users");
		//$muid=$data[muid];
		//$this->db->query("INSERT INTO `shixiquan`.users SET uid='$muid',name='$username', pass='$password',mail='$email',signature_format='plain_text',created='".$this->base->time."', access= '".$this->base->time."',login='".$this->base->time."',status='1',timezone='Asia/Shanghai', language='zh-hans'，picture=0,init='$email'");
